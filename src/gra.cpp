...
void szachownica::zmien_stan_komorek(std::vector< std::vector <istota> > &plansza)
{
    ...
    for (w = 0; w < rozmiar; w++)
    {
        for (k = 0; k < rozmiar; k++)
        {
            ...
            //   starzej� si� (starszy pozostaje taki sam)
            ...
            if ((plansza[w][k].stan==0 || plansza[w][k].stan==2) && sasiedzi.Ball==4 && sasiedzi.Ball==sasiedzi.Aall)
            {
                ...
                else if (sasiedzi.Astarszy>sasiedzi.Bstarszy)
                {
                    ...
                    {
                        plansza[w][k].stan=4;
                    } else if (sasiedzi.Bdorosly<sasiedzi.Adorosly)
                    {
                        plansza[w][k].stan=3;
                    }
                }
            ...
            //   jesli nie ma, pozostaje zywy i starzeje si� (starszy pozostaje taki sam)
...
void* watek_autoiteracja(void *arg)
{
    ...
    if (liczbaA==0)
        komunikat2("Dominacje osiagnela populacja B.");
    else if (liczbaB==0)
        komunikat2("Dominacje osiagnela populacja A.");
